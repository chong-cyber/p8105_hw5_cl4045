...
# Problem 3
```{r}
# function used to conduct t.test given the fixed n and fixed sigma
sim_pval = function(mu_input) {
  sim_data  = 
    tibble(
      x = rnorm(n = 30, mean = mu_input, sd = 5)
    ) %>%
    ...
    select(estimate, p.value, decision)
}

# writing the simulation function
sim_results = function(x){
  rerun(5000,sim_pval(mu=x)) %>%
  bind_rows()
}

set.seed(1984)

# conduct the simulation with mu at 0
sim_results(0)

# conduct the simulation with mu from 0:6
all_result_df = 
  tibble(mu = 0:6,
